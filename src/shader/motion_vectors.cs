...
layout(binding = 0, rg16f) uniform image2D motion_vectors_from_cur_to_last;
...
layout(local_size_x = 16, local_size_y = 16) in;
 void main() {
    ...
    if (depth_cur <= 0.0001){
        imageStore(motion_vectors_from_cur_to_last, gid, vec4(1,0,0,1)); //vec4(max_index-2*gid,0,1));
        return;
    }
